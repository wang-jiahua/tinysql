...
func (s *joinReorderDPSolver) solve(joinGroup []LogicalPlan, eqConds []expression.Expression) (LogicalPlan, error) {
	...
	adjMatrix := make([][]int, len(s.curJoinGroup)) // adjacent matrix
	...
	for i := 0; i < len(joinGroup); i++ {
		...
		nodeIDMask := uint(0) // the subgraph corresponding to node i
		...
		for i := len(totalNonEqEdges) - 1; i >= 0; i-- {
            if totalNonEqEdges[i].nodeIDMask&nodeIDMask != totalNonEqEdges[i].nodeIDMask {
				// current nonequal edge does not exist in the subgraph corresponding to node i
				continue
			}
            // current nonequal edge exists in the subgraph corresponding to node i
            newMask := uint(0) // visit indices
        ...
        // dynamic programming on the subgraph
...
func (s *joinReorderDPSolver) doDP (visitID2NodeID, nodeID2VisitID []int, joinGroup []LogicalPlan, totalEqEdges []joinGroupEqEdge, totalNonEqEdges []joinGroupNonEqEdge) (LogicalPlan, error) {
...
func (s *joinReorderDPSolver) getConnectedNodes(leftMask, rightMask uint, nodeID2VisitID []int, totalEqEdges []joinGroupEqEdge, totalNonEqEdges []joinGroupNonEqEdge) ([]joinGroupEqEdge, []expression.Expression) {
	...
	for _, edge := range totalEqEdges {
		...
		if ((leftMask&(1<<lIndex)) > 0 && (rightMask&(1<<rIndex)) > 0) || ((leftMask&(1<<rIndex)) > 0 && (rightMask&(1<<lIndex)) > 0) {
            // the edge connects left component and right component
