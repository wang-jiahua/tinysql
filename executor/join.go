...
func (e *HashJoinExec) fetchAndBuildHashTable(ctx context.Context) error {
	...
	innerKeyColIdxs := make([]int, len(e.innerKeys))
	for i := range e.innerKeys {
		innerKeyColIdxs[i] = e.innerKeys[i].Index
	}
	allTypes := e.innerSideExec.base().retFieldTypes
	hCtx := &hashContext{allTypes: allTypes, keyColIdx: innerKeyColIdxs}
...
func (e *HashJoinExec) runJoinWorker(workerID uint, outerKeyColIdx []int) {
	...
	var (
		...
		hCtx             = &hashContext{allTypes: retTypes(e.outerSideExec), keyColIdx: outerKeyColIdx}
		emptyOuterResult = &outerChkResource{dest: e.outerResultChs[workerID]}
	)
	...
	for ok := true; ok; {
		// get an outer chunk from outerResultChs[workerID]
		select {
		case <-e.closeCh:
			return
		...
		// probe the hash table to get the join result
		ok, joinResult = e.join2Chunk(workerID, outerResult, hCtx, joinResult, selected)
		if !ok {
			break
		}
		// put the outer chunk back to outerChkResourceCh
		outerResult.Reset()
	...
	if joinResult.err != nil || (joinResult.chk != nil && joinResult.chk.NumRows() > 0) {
		// joinResult is not empty, send it to e.joinResultCh
		e.joinResultCh <- joinResult
	} else if joinResult.chk != nil && joinResult.chk.NumRows() == 0 {
		// joinResult is empty, put it back to e.joinChkResourceCh[workerID]
